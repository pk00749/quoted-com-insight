...
- 定时调度：
  ...
  - 若后续支持多时间点，解析为列表排序后使用循环创建异 AsyncTask。
...
## 任务12: 在保证服务可用前提下缩小 Docker 镜像体积（仅文档，先不实现代码）

### 📋 目标
在不影响服务稳定运行（FastAPI + AKShare + Playwright + pdfplumber）的前提下，显著减少镜像体积并提升构建与拉取速度。

### 🔧 优化方向（方案库）
1) 基础镜像选择
- 方案A：沿用 Playwright 官方基础镜像，但只安装 Chromium，避免安装全部浏览器。
- 方案B：多阶段构建 + python:3.x-slim，使用 `playwright install --with-deps chromium`，仅携带运行所需依赖。

2) 多阶段构建与层优化
- 使用 builder 阶段安装编译依赖、构建 wheels 与 venv；runtime 阶段仅复制 venv、应用代码与 /ms-playwright 中的 Chromium 运行时。
- 合并 RUN 指令、删除 apt 缓存（rm -rf /var/lib/apt/lists/*）、pip 缓存（--no-cache-dir）。
- 仅 COPY 生产必需文件（使用 .dockerignore 排除 tests、docs、.git、示例数据等）。

3) Python 依赖与 Playwright 浏览器
- 固定 requirements.txt 版本，尽量使用 manylinux 预编译轮子，避免编译产生的大型层。
- 只安装 `chromium` 浏览器；不安装 webkit/firefox。
- 清理 Playwright 额外不需要的资源与缓存目录（如测试用模板等）。

4) 系统依赖精简
- apt-get 使用 `--no-install-recommends`；仅安装最小运行集（curl、字体子集、pdfplumber 依赖）。
- 中文字体选择体积较小的字体包（如 `fonts-noto-cjk` 子集或替代），避免大型全量字体。
- 根据实际需求选择是否保留 tzdata，本地化仅保持必要的 zh_CN.UTF-8。

5) 构建与拉取加速
- 使用国内 APT 源（已在任务4中规划）与 pip 源（如阿里/清华镜像），并在构建后还原源配置/清理缓存。
- 利用 Docker BuildKit 与层缓存；可在 CI 中启用缓存（registry cache）。

### 🧪 验收标准
- [ ] 保持所有功能可用：/health、公告抓取、PDF解析、Playwright 渲染、AI总结调用均正常。
- [ ] 仅安装 Chromium，Playwright 正常运行、权限正确（非 root 用户可运行）。
- [ ] 镜像总体积较当前基线下降 ≥30%（或达到预期阈值，例如 ≤1.5GB，具体以基线为准）。
- [ ] 构建产物中不包含 tests、docs、.git、开发脚本等非运行必需内容。
- [ ] 运行容器 `apt` 与 `pip` 缓存被清理；无冗余包管理器缓存。
- [ ] CI/CD 中构建时长与拉取时长较基线下降（记录一次数据以备对比）。

### 📎 交付物（文档与示例）
- 优化后的 Dockerfile（多阶段）与 .dockerignore 示例。
- 构建体积与时间对比记录（优化前/后）。
- 常见问题记录：字体缺失、Playwright 权限、证书目录挂载等。

### ⚠️ 注意事项
- 不牺牲稳定性与可维护性：必要依赖不可删除。
- 若因精简导致功能异常（如 PDF 中文乱码、动态渲染失败），需回滚或补充依赖。
- 关注不同平台（amd64/arm64）镜像体积差异与兼容性。
